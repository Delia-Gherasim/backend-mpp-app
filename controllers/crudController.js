...
const ClientManager = require("../repository/clientManager");
...
module.exports = {
  ...
  validateData: function (device) {
    console.log(device);
    if (
      device.getCategory().toLowerCase() !== "electronics" &&
      device.getCategory().toLowerCase() !== "appliances" &&
      device.getCategory().toLowerCase() !== "equipment"
    )
